...
func prepareTextures() {
	...
	dc.SetRGBA255(int(red.R), int(red.G), int(red.B), int(red.A))
	dc.Fill()
	dc.DrawRectangle(float64(width), 0, float64(width), float64(height))
	dc.SetRGBA255(int(green.R), int(green.G), int(green.B), int(green.A))
	dc.Fill()
	dc.DrawRectangle(0, float64(height), float64(width), float64(height))
	dc.SetRGBA255(int(yellow.R), int(yellow.G), int(yellow.B), int(yellow.A))
	dc.Fill()
	dc.DrawRectangle(float64(width), float64(height), float64(width), float64(height))
	dc.SetRGBA255(int(blue.R), int(blue.G), int(blue.B), int(blue.A))
	dc.Fill()
	...
	orPanic(w.Flush())
	pngBytes = bytesBuffer.Bytes()
	selectedCombinedTexture = rl.LoadTextureFromImage(rl.LoadImageFromMemory(".png", pngBytes, int32(len(pngBytes))))
}
